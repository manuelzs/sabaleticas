...
- **Land: 170.73 ha** `[cadastre]` — **two parcels under one matrícula 023-16153**: *AP 1 SABALETITAS* 151.85 ha (La Pintada) + *LO 1 EL GUAICO* 18.88 ha (Santa Bárbara). The farm **straddles a municipal boundary**; always query the cadastre by **matrícula**, never by municipality, or it silently truncates. The 2003 plan's Lote No. 1 said 193.41 ha, but that plan is **demonstrably unreliable where it crosses forest** (up to 173 m off), so the 22.68 ha difference is a *disagreement between two imperfect documents*, not a known missing parcel. **Herd:** ~266 head (stale, selling) → **~1.9/ha** on potrero. Never quote a per-hectare figure without saying which area it uses.
