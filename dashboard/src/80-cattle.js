  ...
  box.innerHTML=`
    ...
    ${(()=>{ const S=herdSeries(); if(!S) return '';
      ...
      <div class="legend">
        ...
        ${S.sinConteo?`<span><i style="background:#ff9100"></i>sin conteo · ${
          S.cSal} salidas${S.cEnt?', '+S.cEnt+' entradas':''}</span>`:''}
        ${S.sesgo?`<span style="margin-left:auto;color:${S.sesgo.col}">${S.sesgo.t}</span>`:''}
        </div>`;})()}
...
function herdSeries(){
  ...
  /* Which way the unknown movements bias the line — derived, not asserted. If they all
     leave, the real herd is lower than drawn; if they all arrive, higher; if mixed, the
     error has no known sign and we say exactly that. */
  const sesgo = !ciegos.length ? null
    : cEnt===0 ? {t:'la caída real es mayor', col:'#ff9100'}
    : cSal===0 ? {t:'el hato real es mayor',  col:'#ff9100'}
    : {t:'sesgo sin signo conocido', col:'#8b98a5'};
  return {pts, ciegos, desde, hasta, anchor:{t:t0, n:n0}, sesgo, cSal, cEnt,
