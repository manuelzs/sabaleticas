  ...
  for(const L of D.layers){
    ...
    if(view.s>labelZoom) for(const f of L.features){
      ...
      if(isPoly){
        ...
        cx.textAlign='left';
        let wmax=0;
        cx.font='bold 19px system-ui'; wmax=cx.measureText(name).width;
        if(sub){ cx.font='bold 13px system-ui'; wmax=Math.max(wmax,cx.measureText(sub).width); }
        if(sub2){ cx.font='11px system-ui'; wmax=Math.max(wmax,cx.measureText(sub2).width); }
        labelBoxes.push({L,f,x0:s0[0]-wmax/2-6, x1:s0[0]+wmax/2+6,
                             y0:s0[1]-19, y1:s0[1]+dy+(sub2?19:6)});
      } else {
