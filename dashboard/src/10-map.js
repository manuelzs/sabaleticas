...
/* ---- hover a polygon's LABEL, light up the polygon ---- */
/* The boxes are collected while drawing, because that is the only place the label's real
   position and size are known — they depend on the font and on the zoom. */
let labelBoxes=[], hoverL=null;
function findLabel(px,py){
  for(let i=labelBoxes.length-1;i>=0;i--){          // last drawn is on top
    const b=labelBoxes[i];
    if(px>=b.x0&&px<=b.x1&&py>=b.y0&&py<=b.y1) return b;
  }
  return null;
}
function drawHoverPoly(){
  if(!hoverL || !hoverL.L.on) return;
  const {L,f}=hoverL, col=f.col||L.colour;
  cx.save();
  cx.lineJoin='round'; cx.lineCap='round';
  const rings = f.t==='Polygon' ? [f.c[0]] : f.c.map(p=>p[0]);
  cx.beginPath();
  for(const r of rings){
    r.forEach((p,k)=>{const q=toScreen(p[0],p[1]); k?cx.lineTo(q[0],q[1]):cx.moveTo(q[0],q[1]);});
    cx.closePath();
  }
  cx.fillStyle=lighten(col,.5).replace('rgb(','rgba(').replace(')',',.18)');
  cx.fill();
  cx.shadowColor=col; cx.shadowBlur=14;
  cx.strokeStyle=lighten(col,.55); cx.lineWidth=L.width+3;
  cx.stroke();
  cx.restore();
}
...
function draw(){
  ...
  for(const L of D.layers){
    ...
    // Potreros are the exception: view.s is the farm's full width in pixels, so a 3000
    // threshold hid every name until you were already zoomed PAST the whole property —
    // exactly when you are trying to judge whether one needs subdividing. They stay on
    // from 1000 and shrink instead of vanishing.
    const isPot = L.name.indexOf('Potreros')===0;
    const labelZoom = isPot ? 1000 : (L.name.indexOf('Vecinos')===0) ? 700 : 3000;
    if(view.s>labelZoom) for(const f of L.features){
      ...
      if(isPoly){
        ...
        // 11 px right at the threshold, full 19 px once properly zoomed in.
        const kz = isPot ? Math.max(0,Math.min(1,(view.s-1000)/3000)) : 1;
        const fz = Math.round(11+8*kz);
        const showSub2 = !isPot || view.s>2000;    // the area is reference, drop it first
        cx.textAlign='center';
        cx.font=`bold ${fz}px system-ui`; cx.lineWidth=Math.max(3,fz*0.26);
        ...
        if(sub2 && showSub2){
          cx.font=`${Math.round(9+2*kz)}px system-ui`; cx.lineWidth=3.5;
          cx.strokeStyle='rgba(0,0,0,.9)'; cx.strokeText(sub2,s0[0],s0[1]+dy+15);
          cx.fillStyle='#cfd8dc'; cx.fillText(sub2,s0[0],s0[1]+dy+15);
        }
        cx.textAlign='left';
        let wmax=0;
        cx.font=`bold ${fz}px system-ui`; wmax=cx.measureText(name).width;
        if(sub){ cx.font='bold 13px system-ui'; wmax=Math.max(wmax,cx.measureText(sub).width); }
        if(sub2&&showSub2){ cx.font=`${Math.round(9+2*kz)}px system-ui`;
          wmax=Math.max(wmax,cx.measureText(sub2).width); }
        labelBoxes.push({L,f,x0:s0[0]-wmax/2-6, x1:s0[0]+wmax/2+6,
                             y0:s0[1]-fz, y1:s0[1]+dy+((sub2&&showSub2)?fz:6)});
      } else {
...
addEventListener('mousemove',e=>{
  ...
  if(!is3d){
    ...
    const hl=findLabel(e.clientX,e.clientY);
    if((hl?hl.f:null)!==(hoverL?hoverL.f:null)){ hoverL=hl; draw(); }
    if(!edgeMode) cv.style.cursor = hoverL?'pointer':'';
    if(edgeMode){
