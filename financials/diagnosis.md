...
|---|---|---|---|
| **H1** | **Low turnover** — cattle accumulate instead of cycling through. Costs are monthly; sales are rare. **Leading suspect, NOT confirmed** ([`../herd/turnover.md`](../herd/turnover.md)): an earlier age-pyramid "confirmation" was retracted because SINIGAN per-animal ages are unreliable. What stands: adequate stocking (~1.33/ha) + ~4.5 small-lot guías/month. | The herd is 256 females, few sales events; capital and pasture likely tied up. | **Owner's lot records** (entry/exit dates → days-on-farm, head in vs. out) — the reliable source, not SINIGAN ages. |
